...
// Most of this code is taken straight from the Adafruit WS2812b example(s), so
// go there for how to do the stuff :). Just adding in code to allow for
// changing effects/settings live so it can be used as an ARGB controller for
// computers with out the port on the motherboard
...
enum settingList { SETTING_BRIGHTNESS, SETTING_DELAY, SETTING_LENGTH };
...
uint16_t chaseStart =
    0;  // Variable to hold the current start LED of the chase effect
...
void effectChase() {
  strip.clear();
  uint16_t stripSize = strip.numPixels();
  int ledNum;
  for (uint16_t i = 0; i < chaseSize; i++) {  // For each pixel in strip...
    ...
    case 'E':
      ...
      switch (commandBytes[1]) {
        case EFFECT_SOLID:  // Solid color effect
          solidColor =
              strip.Color(commandBytes[2], commandBytes[3], commandBytes[4]);
